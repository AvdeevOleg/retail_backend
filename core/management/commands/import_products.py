...
from core.models import Category, Product, ProductPrice, Shop
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("yaml_file", type=str,
                            help="Путь к YAML файлу для импорта")
    ...
    def handle(self, *args, **kwargs):
        ...
        shop, created = Shop.objects.get_or_create(
            name=shop_name, defaults={"file_url": shop_file_url}
        )
        if created:
            self.stdout.write(self.style.SUCCESS(
                f"Магазин {shop.name} создан"))
        else:
        ...
        for good in goods_data:
            ...
            if not category:
                self.stdout.write(
                    self.style.WARNING(
                        f"Категория с id {cat_id} не найдена. "
                        f"Пропуск товара {good.get('name')}"
                    )
                )
                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Товар {product.name} импортирован/обновлён")
            )
